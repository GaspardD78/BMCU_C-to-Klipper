...
def test_wchisp_failure_falls_back_to_dfu():
    script = textwrap.dedent(
        f"""
        export FLASH_AUTOMATION_FLASH_USB_SCRIPT=/bin/true
        source "{SCRIPT_PATH}"
        flash_automation_initialize
        trap - ERR
        trap - EXIT
...
def test_wchisp_failure_falls_back_to_serial_when_no_dfu():
    ...
    assert serial_source != ""
    assert "Bascule automatique vers le flash série" in result.stdout


def test_wchisp_failure_dry_run_defers_serial_validation():
    script = textwrap.dedent(
        f"""
        source "{SCRIPT_PATH}"
        flash_automation_initialize
    ...
    assert fallback_method == "serial"
    assert serial_device == "/dev/ttyUSB-auto"
    assert "validation du port série différée" in result.stdout
